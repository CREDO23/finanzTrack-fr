...
export default function Layout({ children }: { children: ReactNode }): JSX.Element {
  ...
  const transactionsCategoryItems: MenuProps['items'] = [
    {
      key: '1',
      label: (
        <Link href={'/transactions/categories/expenses/new'}>Expense</Link>
      ),
      icon: <span className=' text-cred text-lg'><BiSolidArrowFromBottom/></span>,
      onClick : () => dispatchView({type : ViewActionType.SET_NAVIGATION, payload : false})
    },
    {
      key: '2',
      label: (
        <Link href={'/transactions/categories/incomes/new'}>Income</Link>
      ),
      icon: <span className=' text-cgreen text-lg'><BiSolidArrowFromTop /></span>,
      onClick : () => dispatchView({type : ViewActionType.SET_NAVIGATION, payload : false})
    },
  ];
  ...
  return (
    <div className="w-full  h-full  flex flex-col gap-4">
      {pathname.split('/')[pathname.split('/').length - 1] != 'new' &&  !pathname.split('/').includes('categories') && (
        <>
